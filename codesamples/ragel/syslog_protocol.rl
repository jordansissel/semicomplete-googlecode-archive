%%{
  ...
  month = ( "Jan" | "Feb" | "Mar" | "Apr" | "May" | "Jun"
          ...
          #>{mark("month")}
          #%{ @month = popstring("month") } ;
          ;

  day = ((" "? [1-9]) | ([12] [0-9]) | ("3" [01])) #>{mark("day")} %{ @day = popstring("day") };
  ;
  hour = (([01] [0-9]) | "2" [0-4]) #>{mark("hour")} %{ @hour = popstring("hour") };
  ;
  minute = ([0-5][0-9]) #>{mark("minute")} %{ @minute = popstring("minute") };
  ;
  second = ([0-5][0-9]) #>{mark("minute")} %{ @second = popstring("second") };
  ;

  time = ( hour ":" minute ":" second );
  timestamp = ( month " " day " " time ) >{mark("timestamp")} %{ @timestamp = popstring("timestamp") };
...
if __FILE__ == $0
  ...
  puts "#{RUBY_PLATFORM}/#{RUBY_VERSION}: duration: #{duration} / rate: #{count / duration} / iterations: #{count}"
end
