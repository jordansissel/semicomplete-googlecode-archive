...
int master(char* fn_case,int s_cases,int s_nodesizes,int s_nodenames,int pass_pso_steps,int pass_neighsize,int pass_NumParticles)  {
	...
	char fn_case_base[255];
	char summary_filename[255];
	char ArrowFile[255]; // for Arrow_* _* _*.dot
	char ArrowArrayFile[255]; // for Arrow_*_*_*.dat

	pso_steps=pass_pso_steps;	
	NumParticles=pass_NumParticles;
	neighsize=pass_neighsize;                 //0 is global
	stpcpy(fn_case_base,"");
	strncat(fn_case_base,fn_case,(strlen(fn_case)-4));
	sprintf(best_filename,"Best_%s_%d_%d_%d.txt",fn_case_base,pso_steps,NumParticles,neighsize);
	sprintf(summary_filename,"Summary_%d_%d_%d.txt",pso_steps,NumParticles,neighsize);
	////printf("nodenames file= %s\n",best_filename);
	printf("summary file= %s\n",summary_filename);

	sprintf(ArrowFile,"Arrow_%s_%d_%d_%d.dot",fn_case_base,pso_steps,NumParticles,neighsize);
	sprintf(ArrowArrayFile,"Arrow_%s_%d_%d_%d.dat",fn_case_base,pso_steps,NumParticles,neighsize);
	...
	do {
		...
		while (ip!=theParticles.end() && rank < ntasks) {
			...
			ParticleForArrow=TheBestOneEver;
			...
			fclose(summary_file);
			masterAgent.PrintNetworkToFile(ArrowFile, &ParticleForArrow);


			FILE* Arr_File;

			int mm;
			if((Arr_File=fopen(ArrowArrayFile,"w"))==NULL) {
				printf("Cannot open the file = %s",ArrowArrayFile);
				exit(1);
			}
			else {
				for (mm=0;mm<ss;mm++) {
